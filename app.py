...
<style>
    ...
    .sentiment-positive {
        ...
        border-radius: 12px; padding: 1rem; text-align: center; color: white; font-weight: 600;
    }
    .sentiment-negative {
    ...
    .section-header {
        font-size: 1.4rem; font-weight: 600; color: #667eea;
        border-left: 4px solid #764ba2; padding-left: 1rem; margin: 2rem 0 1rem 0;
    }
    .predict-box {
    ...
    .stTextArea textarea {
        background: rgba(255,255,255,0.05); border: 1px solid rgba(102,126,234,0.4);
    ...
    .footer {
        ...
        padding: 2rem 0; border-top: 1px solid rgba(255,255,255,0.1); margin-top: 3rem;
    }
</style>
...
if page == "🏠 Overview":
    ...
    with c1:
        ...
        fig = go.Figure(go.Pie(
            labels=sc.index.str.capitalize(), values=sc.values, hole=0.6,
            marker_colors=[COLORS.get(s,"#667eea") for s in sc.index], textfont_size=14))
        fig.update_layout(**PT, height=350,
            annotations=[dict(text="Sentiment",x=0.5,y=0.5,font_size=16,showarrow=False,font_color="white")])
    ...
    with c2:
        ...
        fig = go.Figure(go.Bar(
            x=[f"⭐ {i}" for i in rc.index], y=rc.values,
    ...
    with c2:
        ...
        for sent,color in COLORS.items():
            ...
            fig.add_trace(go.Violin(y=subset, name=sent.capitalize(),
...
# ════════════════════════════════════════════════════════════════
# PAGE 2 — LIVE PREDICTOR (VADER based — no TF needed)
# ════════════════════════════════════════════════════════════════
    ...
    review_input = st.text_area("📝 Enter a product review:",
        placeholder="e.g. The battery life is amazing but screen quality could be better.",
        height=150)
    ...
    if predict_btn and review_input.strip():
        with st.spinner("🔄 Analyzing..."):
            ...
            if compound >= 0.05:
                prediction = "Positive"
                proba = [max(0.0, 0.1 - compound*0.05),
                         max(0.0, 0.2 - compound*0.1),
                         min(1.0, 0.5 + compound*0.5)]
            elif compound <= -0.05:
                prediction = "Negative"
                proba = [min(1.0, 0.5 + abs(compound)*0.5),
                         max(0.0, 0.2 - abs(compound)*0.1),
                         max(0.0, 0.1 - abs(compound)*0.05)]
            else:
                prediction = "Neutral"
                proba = [0.2, 0.6, 0.2]
            total_p = sum(proba)
        ...
        st.markdown(f"""<div class="{cls}" style="font-size:1.5rem;padding:1.5rem;margin:1rem 0;">
            {emoji} Predicted Sentiment: <b>{prediction}</b></div>""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    for col,val,label in zip([c1,c2,c3,c4],
        [f"{metrics[\'accuracy\']*100:.2f}%", f"{metrics[\'f1_score\']*100:.2f}%",
    ...
    c1,c2,c3,c4 = st.columns(4)
    for col,val,label in zip([c1,c2,c3,c4],
        [f"{metrics[\'precision\']*100:.2f}%", f"{metrics[\'recall\']*100:.2f}%",
    ...
    with c1:
        ...
        fig.add_trace(go.Scatter(x=epochs,y=history["val_accuracy"],name="Val Acc",
                                  line=dict(color="#38ef7d",width=2.5,dash="dash")),row=1,col=1)
        fig.add_trace(go.Scatter(x=epochs,y=history["loss"],name="Train Loss",
                                  line=dict(color="#f45c43",width=2.5)),row=2,col=1)
        fig.add_trace(go.Scatter(x=epochs,y=history["val_loss"],name="Val Loss",
                                  line=dict(color="#ffd200",width=2.5,dash="dash")),row=2,col=1)
    ...
    c1,c2,c3,c4 = st.columns(4)
    for col,val,label in zip([c1,c2,c3,c4],
        [f"{metrics[\'training_time\']:.1f}s", f"{metrics[\'inference_time\']*1000:.1f}ms",
    ...
    p,r,f,_ = precision_recall_fscore_support(
        df["label"], df["predicted_sentiment"].map({"negative":0,"neutral":1,"positive":2}), average=None)
...
# ════════════════════════════════════════════════════════════════
    ...
    fig = go.Figure(go.Heatmap(
        z=hm.values, x=hm.columns.str.capitalize(), y=hm.index.str.capitalize(),
    ...
    fig = go.Figure(go.Bar(
        x=ap.values, y=ap.index.str.capitalize(), orientation="h",
...
# ════════════════════════════════════════════════════════════════
    ...
    for _,row in df.iterrows():
        ...
        for asp in aspects:
            if asp in text:
                aspect_data.append({
                    "aspect":asp.title(),"sentiment":row["sentiment"],
    ...
    with c2:
        ...
        fig = go.Figure(go.Bar(
            x=av.values, y=av.index, orientation="h",
    ...
    fig.update_layout(**PT, height=400, xaxis_title="Aspect", yaxis_title="Count")
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig = go.Figure(go.Scatterpolar(
        r=rd["vader"], theta=rd["aspect"], fill="toself",
    ...
    st.markdown("<div class=\'section-header\'>Product × Aspect Heatmap</div>", unsafe_allow_html=True)
    pa = adf.groupby(["product","aspect"])["vader"].mean().unstack(fill_value=0)
    fig = go.Figure(go.Heatmap(
        z=pa.values, x=pa.columns, y=pa.index.str.capitalize(),
...
# ════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        arch = pd.DataFrame({
            ...
            "Value":["Bidirectional LSTM + Attention",
                     ...
                     str(hyperparams["max_len"]), "128 units (×2=256)","64 units (×2=128)",
    ...
    layers = ["Input (seq=100)","Embedding (10K×100)","Dropout 0.3",
    ...
    for i,(layer,color) in enumerate(zip(layers,colors_a)):
        fig.add_trace(go.Scatter(
            x=[0.5], y=[len(layers)-i], mode="markers+text",
    ...
    fig.update_layout(**PT, height=700,
...
st.markdown("""
<div class="footer">
    🧠 SentIQ — Sentiment Intelligence Dashboard | Bi-LSTM + Attention | TensorFlow + Streamlit<br>
